        ...
        function getAvailableTemplates() {
            return state.agentTemplates.length > 0 ? state.agentTemplates : getFallbackTemplates();
        }

        function buildAgentSelectionValue(profileName, templateName) {
            return `${encodeURIComponent(profileName || "")}::${encodeURIComponent(templateName || "default")}`;
        }

        function parseAgentSelectionValue(value) {
            const [profileName = "", templateName = "default"] = String(value || "").split("::");
            return {
                profileName: decodeURIComponent(profileName || ""),
                templateName: decodeURIComponent(templateName || "default"),
            };
        }

        function updateProfileBadge() {
            const profileName = state.settings.profileName || "";
            const templateName = state.settings.template || "default";
            els.profileBadge.textContent = profileName ? `${profileName} (${templateName})` : templateName;
        }
        ...
        function renderAssignedProfiles() {
            ...
            els.profileBadge.textContent = selectedProfileName || state.settings.template || "default";
        }
        ...
        els.composerProfileSelect.addEventListener("change", async () => {
            const selection = parseAgentSelectionValue(els.composerProfileSelect.value.trim());
            state.settings.profileName = selection.profileName;
            state.settings.template = selection.templateName || "default";
            els.settingProfile.value = state.settings.profileName;
            els.settingTemplate.value = state.settings.template;
            updateProfileBadge();
            await persistSettings();
            if (state.ws) {
                state.ws.close();
        ...
        els.settingProfile.addEventListener("change", () => {
            const profileName = els.settingProfile.value.trim();
            const templateName = els.settingTemplate.value || state.settings.template || "default";
            state.settings.profileName = profileName;
            state.settings.template = templateName;
            els.composerProfileSelect.value = profileName ? buildAgentSelectionValue(profileName, templateName) : "";
            updateProfileBadge();
        });
        els.settingTemplate.addEventListener("change", () => {
            const profileName = els.settingProfile.value.trim() || state.settings.profileName || "";
            const templateName = els.settingTemplate.value || "default";
            state.settings.profileName = profileName;
            state.settings.template = templateName;
            els.composerProfileSelect.value = profileName ? buildAgentSelectionValue(profileName, templateName) : "";
            updateProfileBadge();
        ...
        function renderAssignedProfiles() {
            const selectedProfileName = state.settings.profileName || "";
            const selectedTemplateName = state.settings.template || "default";
            const defaultLabel = getLocale() === "en" ? "Default assigned profile" : "Ø§Ù„Ù…Ù„Ù Ø§Ù„Ø§ÙØªØ±Ø§Ø¶ÙŠ Ø§Ù„Ù…Ø®ØµØµ";
            const profileOptionsHtml = [`<option value="">${defaultLabel}</option>`]
                .concat(state.assignedProfiles.map((profile) => `
                    <option value="${escapeHtml(profile.name)}" ${selectedProfileName === profile.name ? "selected" : ""}>
                        ${escapeHtml(profile.name)}
                    </option>
                `))
                .join("");
            const composerOptionsHtml = [`<option value="">${defaultLabel}</option>`]
                .concat(state.assignedProfiles.flatMap((profile) => (
                    getAvailableTemplates().map((template) => {
                        const value = buildAgentSelectionValue(profile.name, template.name);
                        const label = `${profile.name} (${template.name})`;
                        const isSelected = selectedProfileName === profile.name && selectedTemplateName === template.name;
                        return `
            ...
            els.settingProfile.innerHTML = profileOptionsHtml;
            els.composerProfileSelect.innerHTML = composerOptionsHtml;
            els.settingProfile.value = selectedProfileName;
            els.settingTemplate.value = selectedTemplateName;
            els.composerProfileSelect.value = selectedProfileName
                ? buildAgentSelectionValue(selectedProfileName, selectedTemplateName)
                : "";
            ...
            els.settingProfileStatus.textContent = `${state.assignedProfiles.length} assigned profile(s) available with ${getAvailableTemplates().length} template option(s) each.`;
            updateProfileBadge();
        }

        async function loadAgentTemplates() {
            if (!state.settings.token) {
                state.agentTemplates = getFallbackTemplates();
                renderAssignedProfiles();
                return;
            }

            try {
                const data = await apiRequest("/auth/agent-templates");
                state.agentTemplates = Array.isArray(data.templates) && data.templates.length > 0
                    ? data.templates
            ...
            if (!getAvailableTemplates().some((template) => template.name === (state.settings.template || "default"))) {
                state.settings.template = getAvailableTemplates()[0]?.name || "default";
            }
            renderAssignedProfiles();
        }
        ...
        async function saveSettings() {
            ...
            state.settings.commandMode = els.composerCommandSelect?.value || "queue";
            state.settings.approvalMode = els.composerApprovalSelect?.value || "ask_for_approval";
            state.settings.profileName = els.settingProfile?.value.trim() || state.settings.profileName || "";
            state.settings.template = els.settingTemplate?.value || state.settings.template || "default";
            state.settings = await window.electronAPI.setSettings({
                ...state.settings,
                apiUrl: els.settingApiUrl.value.trim(),
                token: els.settingToken.value.trim(),
