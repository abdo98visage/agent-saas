...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  refreshSession: (apiUrl) => ipcRenderer.invoke("refresh-desktop-session", apiUrl),
  clearSession: () => ipcRenderer.invoke("clear-desktop-session"),
  activateSession: (apiUrl, token, password) => ipcRenderer.invoke("activate-desktop-session", apiUrl, token, password),
  ...
  readImageFile: (rootPath, filePath) => ipcRenderer.invoke("read-image-file", rootPath, filePath),
  ...
  setDirtyState: (isDirty) => ipcRenderer.invoke("set-renderer-dirty-state", Boolean(isDirty)),
