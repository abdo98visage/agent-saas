...
export default function AgentRuntimePage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!cancelled) {
          toast.error(safeText(getErrorMessage(error, t("Failed to load agent runtime status"))));
          setLoading(false);
        }
      }
    };
  ...
  const runAction = async (action: "install" | "start" | "restart" | "stop" | "repair-sync") => {
    ...
    try {
      ...
      toast.success(t("Agent action requested"));
      await load();
    } catch (error: unknown) {
      toast.error(safeText(getErrorMessage(error, t("Agent action failed"))));
    } finally {
      setBusyAction(null);
    }
  };
  ...
  return (
    <div className="p-8 space-y-6 kos-animate-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight kos-gradient-text">{t("Agent Runtime")}</h1>
          <p className="text-muted-foreground mt-1">{t("Install, health-check, restart, and repair the agent execution layer.")}</p>
        </div>
      ...
      <Card className="kos-card">
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="grid gap-3 md:grid-cols-2">
            <div><span className="text-muted-foreground">{t("Docker image:")}</span> {safeText(status?.docker_image || t("Not reported"))}</div>
          ...
          {status?.message && <p className="text-amber-700 bg-amber-50 border border-amber-100 rounded-md p-3">{safeText(status.message)}</p>}
          <pre className="bg-gray-950 text-gray-100 rounded-md p-4 overflow-auto max-h-80 text-xs">
            {logs.length ? safeText(logs.join("\n")) : t("No logs available.")}
          </pre>
        </CardContent>
      </Card>
    </div>
